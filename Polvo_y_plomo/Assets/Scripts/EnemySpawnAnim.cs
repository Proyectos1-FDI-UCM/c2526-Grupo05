...
public class EnemySpawnAnim : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            _enemyMeleeAttack.enabled = false;
        }
        //string[] anim = (SpawnAnimation.ToString()).Split();
        _spawnTime = Time.time;
        _animator.SetInteger("SpawnType", SpawnAnimation);
    }
    ...
    void FixedUpdate()
    {
        if (Time.time - _spawnTime > SpawnDuration)
        {
            ...
            if (_enemyMeleeAttack != null)
            {
                _enemyMeleeAttack.enabled = true;
            }
        }
    }
    #endregion
